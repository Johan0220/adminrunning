...
function Customer(props:any){ // properties  
    ...
    return (
        <>
                ...
                <td className="px-4 py-3">{users.fullname}</td>        
                <td className="px-4 py-3">{users.email}</td>        
                <td className="px-4 py-3">{users.role}</td>     
                <td className="px-4 py-3">{users.isActived?'Active':'Inactive'}</td>      
                     
                <td className="px-4 py-3 flex justify-center">
                    <OutsideClickHandler onOutsideClick={(actbra)=>{setActbra(!actbra)}}>
                    <button onClick={()=>{setActbra(!actbra)}} id={users.id} data-dropdown-add={users.name} className="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                        <svg className="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                        </svg>
                    </button>

                    <div id={users.name} style={{display:actbra?"block":"none"}} className="none right-2 absolute z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
            ...
            <div id="ComfirmModal" tabIndex={-1} aria-hidden="true" style={{display:delord? "block" : "none"}} className="none bg-smoke overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-100 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div className="inset-35 relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div className="relative p-4 text-center bg-white border-solid border rounded-lg shadow dark:border-gray-600 dark:bg-gray-800 sm:p-5">
                        ...
                        <p className="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete brand {users.name}?</p>
                        <div className="flex justify-center items-center space-x-4">
                            ...
                            <button onClick={()=>{handleDelete(users.id),setDelord(!delord)}} className="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">Yes, I&apos;m sure</button>
                        </div>
                    </div>
                </div>
            </div>
