...
import {default as BrandGird} from "@/components/brand/brandList";
import { Product as ProductType, NewProduct, ProductSearch } from "@/components/types/typeProduct";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { addProduct,loadBrand,loadProduct} from "@/app/services/apiRequest";
...
const Brand = () => {
    ...
    const brands: any = useSelector((state: RootState)=>state.brand.brands.brandList);
    const error: any = useSelector((state: RootState)=>state.brand.brands.error);
    const isFetching :any = useSelector((state: RootState)=>state.brand.brands.isFetching);
    ...
    useEffect(()=>{
        loadBrand(dispatch,tokenStr,page);  
    },[page]);
    return(
        <>
                                    ...
                                    <table className="table-auto w-full left-1/4 text-sm text-left text-gray-500 dark:text-gray-400">
                                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                            <tr>
                                                ...
                                                <th scope="col" className="px-4 py-4">Brand name</th>
                                        ...
                                        <tbody>
                                            {brands.map((b:any,index:number) => (
                                                ...
                                                <tr key={index} className="border-b relative dark:border-gray-700">
                                                    <BrandGird brands={b} index={index}/>
                                                </tr>))
                                            }
                                        </tbody>
                                        :
